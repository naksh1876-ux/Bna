...
OXAPAY_ENABLED = True
OXAPAY_API_KEY = "UQS0E6-UXUKJC-V9TLRN-WFCPVP"
OXAPAY_MERCHANT_ID = "469NON-OAPSCL-7TRM2H-BCCEDT"
...
async def button_callback(query, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Ticket status
            ...
            cat = data[4:]
            # If "All Stock", show ALL products regardless of category
            if cat == "All Stock":
            ...
            if not cat_products:
                ...
                for p in cat_products[:20]:
                    ...
                    text += f"#{p['id']} - {p['name']}\n💰 ${p['price']:.2f} | {stock_status}\n\n"
                    kb.append([InlineKeyboardButton(f"View #{p['id']} - {p['name']}", callback_data=f"view_{p['id']}")])
                kb.append([InlineKeyboardButton("« Back", callback_data="start")])
                await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(kb))
                await query.answer()
        ...
        # View product details
            ...
            stock_text = f"{product['stock']} available" if product['stock'] > 0 else "Out of Stock"
            ...
            product_info = (
                ...
                f"{stock_emoji} Stock: {stock_text}\n"
                f"📂 Category: {product.get('category', 'N/A')}\n"
            )
            
            # Add all specs (Seller, Country, BIN, Brand, Type, Bank, Fullz)
            ...
            if delivery:
                product_info += f"\n📨 Delivery Info:\n"
                product_info += f"Type: {delivery.get('type', 'N/A')}\n"
                if delivery.get('format'):
                    product_info += f"Format: {delivery['format']}\n"
            ...
            if product['stock'] > 0:
                if user['balance'] >= product['price']:
                    kb.append([InlineKeyboardButton("✅ Buy Now", callback_data=f"buy_{pid}")])
                else:
                    kb.append([InlineKeyboardButton("❌ Insufficient Balance", callback_data="wallet")])
            else:
                kb.append([InlineKeyboardButton("❌ Out of Stock", callback_data=f"cat_{product.get('category', '')}")])
            
            kb.append([InlineKeyboardButton("« Back to Category", callback_data=f"cat_{product.get('category', '')}")])
            kb.append([InlineKeyboardButton("« Main Menu", callback_data="start")])
            
            await query.edit_message_text(product_info, reply_markup=InlineKeyboardMarkup(kb))
            await query.answer()
            ...
            if user['balance'] < product['price']:
                ...
                await query.edit_message_text(
                    ...
                    f"Needed: ${product['price'] - user['balance']:.2f}\n\n"
                    f"Please add funds to your wallet.",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("💰 Add Funds", callback_data="wallet")],
                        [InlineKeyboardButton("« Back", callback_data="start")]
                    ])
                )
                return
            ...
            try:
                await query.edit_message_text(
                    "Thank you for your purchase! Check the message above for your delivery.",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Back to Shop", callback_data="start")]]),
                )
            except Exception as e:
                print(f"Failed to edit message: {e}")
            
            # CRITICAL: Always answer the callback query
            ...
            await query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Cancel", callback_data="admin_manage_categories")]]),
            )
            await query.answer()
        elif data == "admin_del_cat_prompt":
            if not is_admin(user_id):
                return
            if not categories:
                await query.answer("No categories to delete", show_alert=True)
                return
            context.user_data['admin_del_cat'] = True
            cat_list = "\n".join(f"• {c}" for c in categories)
            await query.edit_message_text(
                f"🗑️ Delete Category\n\nCurrent categories:\n{cat_list}\n\nEnter the exact category name to delete:",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Cancel", callback_data="admin_manage_categories")]]),
            )
            ...
            await query.edit_message_text(
                "Enter review channel username (e.g. @yourchannel) or ID:",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Cancel", callback_data="admin_settings")]]),
            )
            await query.answer()
            ...
            kb = [
                [InlineKeyboardButton("Create Coupon", callback_data="admin_create_coupon")],
                [InlineKeyboardButton("List Coupons", callback_data="admin_list_coupons")],
                [InlineKeyboardButton("Delete Coupon", callback_data="admin_delete_coupon")],
                [InlineKeyboardButton("Back", callback_data="admin_panel")],
            ]
            await query.edit_message_text("🎟️ Coupon Management", reply_markup=InlineKeyboardMarkup(kb))
            await query.answer()
            ...
            await query.edit_message_text(
                "🎟️ Create New Coupon\n\nStep 1: Enter coupon code (letters/numbers only):",
            ...
            if not coupons:
                ...
                for code, info in coupons.items():
                    ...
                    text += f"{status} {code}: ${info['value']:.2f} ({info['uses']}"
                    if info.get('max_uses'):
                        text += f"/{info['max_uses']}"
                    text += " uses)\n"
            
            await query.edit_message_text(
                text,
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Back", callback_data="admin_coupons")]]),
            )
            await query.answer()
        ...
        # Always answer the query to remove loading state
        try:
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('admin_del_cat') and is_admin(user_id):
        del context.user_data['admin_del_cat']
        if text not in categories:
            await update.message.reply_text(
                f"❌ Category '{text}' not found. Check spelling.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Back", callback_data="admin_manage_categories")]]),
            )
        else:
            # Check if any products use this category
            products_in_cat = [p for p in products if p.get('category') == text]
            if products_in_cat:
                await update.message.reply_text(
                    f"⚠️ Cannot delete '{text}' - it has {len(products_in_cat)} product(s).\n\n"
                    f"Delete or move those products first.",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Back", callback_data="admin_manage_categories")]]),
                )
            else:
